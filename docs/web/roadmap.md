...
**🗂️ Backlog tri thức D4 (card CHỜ LẤP — chưa tới lượt, đừng lấp vội):**
- **Task Map** (framework thay đổi hành vi / comms planning) — lưới **Current→Desired × (Do · Think-Feel-Believe ·
  Insights-Barriers-Drivers)**. Đích: `brain/frameworks/task-map.md`, `goal_type: [content]`, `applies_to: [all]`,
  `composes_with: [[jtbd]]`. **Vai:** khuôn biến *customer-insight* → *brief nội dung* (cầu Insight→D4). **Exemplar
  có nguồn:** Listerine VN (khoá BrandCamp — Mai Hồng Ngọc, ex-Dentsu Redder). ⚠️ **Caveat:** hàng Barriers/Drivers
  giẫm JTBD forces + cột Current→Desired giẫm phễu → định vị là *"cấu trúc dựng brief"*, KHÔNG nhân bản trùng JTBD.
  **Lấp khi:** khởi động D4 tầng chiến lược (SAU D1 → R-1). Nguồn đề xuất: founder gửi 2026-07-09.
