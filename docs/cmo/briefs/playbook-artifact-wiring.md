...
{ "segments": [ {
    ...
    "tiers": {
      "tofu": [ {
        ...
        "cut": "Watch-through>30% & CTR>2%",// ngưỡng đọc tín hiệu go/kill (tương đối)
...
- **W1 — Nối GÓC ĐÁNH vào sản xuất bài:** `gen_calendar_post` (bài lẻ — CHỐT làm trước; `gen_calendar_topics`
  ...
  channels + huong** của Hướng khớp vào prompt làm *"góc đánh tham chiếu — bám góc này, tự viết câu chữ bằng
  giọng Thông điệp"*. **KHÔNG truyền `example` vào như hook.** Degrade: thiếu struct → giữ hành vi cũ (tự chế).
