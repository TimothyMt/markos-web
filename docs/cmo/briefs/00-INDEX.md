...
| **VL1** | `VL1-video-viral-learn.md` | **Nối skill `viral_video_analyzer` ĐÃ CÓ vào web** (nó đang kẹt ở `agents/`) + vá đường tải hỏng. F1 extract(gendownload) · F2 Gemini watcher (thay krillin+ffmpeg, GIỮ prompt 9-section) · F3 ⭐wire web+router · FE · F6 cầu formula→gen bài · F4 stats · F5 cụm ③ | ⬜ brief xong (feasibility ✅ prototype), **chờ Cline** |
